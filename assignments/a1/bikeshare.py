...
class Station(Drawable):
    ...
    bikes_start: int
        number of bikes at the start of the station
    low_avail: list[Tuple(datetime, datetime)]
        list of time intervals when station has less than 5 available bikes
    ...
    def is_full(self):
        """Return whether the station has reached it's capacity or not
        """
        return self.num_bikes == self.capacity

    def is_low_availability(self):
        """Return whether the Station has atmost five bikes available
        """
        return self.num_bikes <= 5

    def is_low_unoccupied(self):
        """Return whether the station has atmost five unoccupied
        or not
        """
...
class Ride(Drawable):
    ...
    def get_position(self, time: datetime) -> Tuple[float, float]:
        ...
        long_v = (long2 - long1)/diff
        lat_v = (lat2 - lat1)/diff
        cur_time = (time - self.start_time).total_seconds()
        cur_long = long1 + (long_v*cur_time)
        cur_lat = lat1 + (lat_v*cur_time)
