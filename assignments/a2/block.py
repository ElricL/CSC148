...
class Block:
    ...
    def rectangles_to_draw(self) -> List[Tuple[Tuple[int, int, int],
        ...
        if len(self.children) == 0:
            block = (self.colour, self.position,
                     (self.size, self.size), 0)
            frame = (FRAME_COLOUR, self.position,
                     (self.size, self.size), 3)
            rectangles.append(block)
            rectangles.append(frame)
        else:
            for child in self.children:
                rectangles += child.rectangles_to_draw()
        if self.highlighted is True:
            frame = (HIGHLIGHT_COLOUR, self.position,
    ...
    def swap(self, direction: int) -> None:
        ...
        """
        if direction == 1 and self.children:
        ...
        elif direction == 0 and self.children:
    ...
    def update_block_locations(self, top_left: Tuple[int, int],
        ...
        if len(self.children) > 0:
            unit = round(size / 2.0)
            self.children[1].update_block_locations(top_left, unit)
            new_top = (top_left[0], top_left[1] + unit)
            self.children[2].update_block_locations(new_top, unit)
            new_top = (top_left[0] + unit, top_left[1])
            self.children[0].update_block_locations(new_top, unit)
            new_top = (top_left[0] + unit, top_left[1] + unit)
            self.children[3].update_block_locations(new_top, unit)
    ...
    def get_selected_block(self, location: Tuple[int, int], level: int) \
        ...
        if level > self.level:
            for block in self.children:
                if block.check_within_block(location):
                    return block.get_selected_block(location, level)
            return self
        else:
            return self

    def check_within_block(self, location: Tuple[int, int]) -> bool:
        """Return wether the location is within this Block on the board.

        Helper method for get_selected_block.
        """
        self_x, self_y = self.position[0], self.position[1]
        block_x, block_y = location[0], location[1]
        if (self.size + self_y) < block_y or \
                (self.size + self_x) < block_x or \
                self_x > block_x:
            return False
        else:
            return True
    ...
    def flatten(self) -> List[List[Tuple[int, int, int]]]:
        ...
        flattened_list = []
        if self.colour:
            unit_size = 2 ** (self.max_depth - self.level)
            while len(flattened_list) < unit_size:
                flattened_list.append([self.colour])
            if unit_size > 1:
                for i in range(len(flattened_list)):
                    flattened_list[i] += [self.colour]*(unit_size-1)
            return flattened_list
        else:
            flattened_left = self.children[1].flatten()
            bottom_left = self.children[2].flatten()
            for i in range(len(flattened_left)):
                flattened_left[i].extend(bottom_left[i])
            flattened_right = self.children[0].flatten()
            bottom_right = self.children[3].flatten()
            for i in range(len(flattened_right)):
                flattened_right[i].extend(bottom_right[i])
...
def random_init(level: int, max_depth: int) -> 'Block':
    ...
    rand = random.random()
    if rand < math.exp(-0.25 * level) and level < max_depth:
        children = []
        for _ in range(4):
            new_block = random_init(level + 1, max_depth)
            new_block.parent = block
            children.append(new_block)
        block.children = children
    else:
        rand_int = random.randint(0, 3)
        block.colour = COLOUR_LIST[rand_int]
    return block
