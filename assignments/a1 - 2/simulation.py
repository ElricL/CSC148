...
class Simulation:
    ...
    active rides:
        A list of all rides that are in progress at the current time in
        the simulation
    events_list:
        A PriorityQueue of events to process in the simulation
    """
    all_stations: Dict[str, Station]
    all_rides: List[Ride]
    visualizer: Visualizer
    active_rides: List[Ride]
    events_list: PriorityQueue['Event']
    ...
    def __init__(self, station_file: str, ride_file: str) -> None:
        ...
        self.active_rides = []
        self.events_list = PriorityQueue()
    ...
    def run(self, start: datetime, end: datetime) -> None:
        ...
        for ride in self.all_rides:
            if ride.start_time > start or start <= ride.end_time <= end:
                start_event = RideStartEvent(self, ride.start_time, ride)
                self.events_list.add(start_event)

        simulation_start = start
        while not start > end:
            self._update_active_rides_fast(start)
            if not start == simulation_start:
                self.low_avail_update()
                self.low_unocc_update()
    ...
    def low_avail_update(self) -> None:
        """Increase station's low availability time if it has atmost
        5 bikes
        """
    ...
    def low_unocc_update(self) -> None:
        """Increase station's low unoccupied time if it has atmost
        5 unoccupied space
        """
        for id_ in self.all_stations:
            station = self.all_stations[id_]
            if station.is_low_unoccupied():
                station.low_unocc += 60.0
    ...
    def _update_active_rides(self, time: datetime) -> None:
        ...
        for ride in self.all_rides:
            if ride.start_time <= time <= ride.end_time \
                    and ride not in self.active_rides:
    ...
    def calculate_statistics(self) -> Dict[str, Tuple[str, float]]:
        ...
        return {
            ...
            'max_time_low_availability': (self.lowest_avail_time()),
            'max_time_low_unoccupied': (self.lowest_unocc_time())
        }

    def max_start(self) -> Tuple[str, int]:
        """Return the station and the amount with most rides that started
        at the station
        """
        max_ = -1
        name = ''
    ...
    def max_end(self) -> Tuple[str, int]:
        """Return the station and the amount with most rides that started
        at the station
        """
        max_ = -1
    ...
    def lowest_avail_time(self) -> Tuple[str, float]:
        """Return the name of the station with longest time in the state of
        "low availability" and the time
        """
        low_time = -1.0
        name = ''
        for id_ in self.all_stations:
            station = self.all_stations[id_]
            if station.low_avail > low_time:
                low_time = station.low_avail
                name = station.name
            elif station.low_avail == low_time:
                if station.name < name:
                    name = station.name
        return name, low_time

    def lowest_unocc_time(self) -> Tuple[str, float]:
        """Return the name of the station with longest time in the state
        of "low unoccupied" and the time
        """
        low_time = -1
        name = ''
        for id_ in self.all_stations:
            station = self.all_stations[id_]
            if station.low_unocc > low_time:
                low_time = station.low_unocc
                name = station.name
            elif station.low_unocc == low_time:
                if station.name < name:
                    name = station.name
        return name, low_time
    ...
    def _update_active_rides_fast(self, time: datetime) -> None:
        ...
        """
        if not self.events_list.is_empty():
            event = self.events_list.remove()
            if event.time > time:
                self.events_list.add(event)
            else:
                self.procces_event(time, event)

    def procces_event(self, time: datetime, event: 'Event') -> None:
        """Helper method to process events lower than the current time
        """
        while event is not None:
            result_events = event.process()
            if isinstance(event, RideStartEvent):
                if event.time < time:  # Don't count start if before simulation
                    event.ride.start.num_bikes += 1
                    event.ride.start.rides_start -= 1
            for new_event in result_events:
                self.events_list.add(new_event)
            event = None
            if not self.events_list.is_empty():
                event = self.events_list.remove()
                if event.time > time:
...
class RideStartEvent(Event):
    ...
    def process(self) -> List['Event']:
        """Process this event by updating ride's start station statistics,
        and adding bike to simulation's active rides list.

        Return a list of ride end events spawned by this event.
        """
        events = []
        if not self.ride.start.num_bikes == 0:
            self.ride.start.num_bikes -= 1
            self.ride.start.rides_start += 1
            self.simulation.active_rides.append(self.ride)
            events.append(RideEndEvent(self.simulation,
                                       self.ride.end_time, self.ride))
        return events
...
class RideEndEvent(Event):
    ...
    def process(self) -> List['Event']:
        """Process this event by updating ride's end station statistics,
        and removing the ride from simulation's active rides list.

        Return an empty list.
        """
        self.simulation.active_rides.remove(self.ride)
        if not self.ride.end.is_full():
            self.ride.end.num_bikes += 1
            self.ride.end.rides_end += 1
        return []
