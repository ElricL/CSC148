...
def find_runs(lst: list) -> List[Tuple[int, int]]:
    ...
    while run_end < len(lst):
        ...
        if lst[run_end] > lst[run_end-1] and run_end != len(lst) - 1:
            run_end += 1
        else:
            if run_end == len(lst) - 1:
    ...
    if len(lst) > 1 and lst[len(lst) - 1] < lst[len(lst) - 2]:
        runs.remove(runs[-1])
        runs.append((run_start - 1, len(lst) - 1))
        runs.append((len(lst) - 1, len(lst)))
    return runs
