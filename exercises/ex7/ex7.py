...
def _anagrams_helper(words: List[str], letter_count: Dict[str, int],
    ...
    for word in words:
        ...
        if _within_letter_count(word, letter_count):
            new_letter_count = new_dict(word, letter_count)
            next_ = _anagrams_helper(words, new_letter_count, limit)
            if len(next_) > 0 and len(next_[0]) > 0:
                word += ' ' + next_[0]
            if check_if_anagram(letter_count, word):
...
def new_dict(word, letter_count):
    """Return a new dictionary with counts decreased based on letters of word
    """
    temp_dict = {}
    for letter in letter_count:
        temp_dict[letter] = letter_count[letter]
    for char in word:
        temp_dict[char] -= 1
    return temp_dict


def check_if_anagram(letter_count, word):
    """Return wether the word is an anagram or not as in all letters are used
    in the dictionary
    """
    for char in letter_count:
        if letter_count[char] != word.count(char):
            return False
    return True
