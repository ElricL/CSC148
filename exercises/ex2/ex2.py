...
class Helicopter(Vehicle):
    ...
    def fuel_needed(self, new_x: int, new_y: int) -> int:
        ...
        indicating that this vehicle may not move to the given position.
        """
        needed = sqrt((new_x - self.position[0]) ** 2 +
...
class UnreliableMagicCarpet(Vehicle):
    ...
    def fuel_needed(self, new_x: int, new_y: int) -> int:
        """fuel is not needed for UnreliableMagicCarpet
        """
        pass

    def move(self, new_x: int, new_y: int) -> None:
        """Move this vehicle to random position within two units from
        target destination
        """
        rand_x = random.randint(new_x-2, new_x+2)
        rand_y = random.randint(new_y-2, new_y+2)
